...
// 7/30/21 constraint List MiniML
        ...
        if (msg.length > 2 && msg.charAt(0) == ':') {
            ...
            switch (msg.charAt(1)) {
                case 'a': //applicative order reduction
                    ...
                    break;
                case 'b': //constraints MiniML as list
                ...
                case 'd': //constraints
                    ...
                    gridOptions.columnApi.autoSizeColumns();
                    break;
                case 'e': //nor order reduction with type, using MiniML
                    var newData = JSON.parse(msg.slice(2));
                    gridOptions.api.setRowData(newData);
                    gridOptions.columnApi.autoSizeColumns();
                    break;
                case 'f': //constraints proof MiniMl
                    var newData = JSON.parse(msg.slice(2));
                    gridOptions.api.setRowData(newData);
                    gridOptions.columnApi.autoSizeColumns();
                    break;
...
function initializeEditorOnly(editor, initialText) {
    bindEvent(window, 'message', function (e) {
        ...
        if (msg.length > 2 && msg.charAt(0) == ':') {
            ...
            switch (msg.charAt(1)) {
                case 'b': //constraints MiniML as list
                     editor.selection.insertHTML(selectedText + "<br /><mark>&nbsp;" + msg.slice(2) + "&nbsp;</mark><br />"); //editor is a global
                break;
...
function norAndTypeMiniML(joditEditor) {
    var prefix = ':e';
    sendMessage(prefix + getStrippedSelection(joditEditor));
}
...
function reduceConstraintsMiniML(joditEditor) {
    //    alert('unified scripts reduceConstraintsJHM'); //called properly
       var prefix = ':f';
       sendMessage(prefix + getStrippedSelection(joditEditor));
   }
function constraintsMiniML(joditEditor) {
    var prefix = ':b';
    sendMessage(prefix + getStrippedSelection(joditEditor));
}

function identifyTypeMiniML(joditEditor) {
        var prefix = ':j';
        sendMessage(prefix + getStrippedSelection(joditEditor));
