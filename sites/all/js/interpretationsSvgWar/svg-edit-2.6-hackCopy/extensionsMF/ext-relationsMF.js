...
/*	  var style=svgCanvas.getStyle();
   ...
   '</marker>'+
   '<marker id="Relation'+name+'" viewBox="0 0 80 80" refX="0" refY="0" markerUnits="strokeWidth" markerWidth="80" markerHeight="80" >'+
 //   '<path d="m0,0l20,10l-20,10l0,-20z"/>'+
   ...
   if (tTob){
    ...
    relation= '<path d="m'+mx+','+my+
            ...
            'id="'+iD+'" '+
            'marker-end="url(#Triangle)" marker-mid="url(#Relation'+name+')" fill="none"  stroke="black"/>';
    
	  return arrowDef+relation;   
        
	};
        
relationsMF.makeCircularRelation=function (name,from,to){
    var relation=null;
    var iD='svg_relation'+name+'Reflexive';
...
/*	  var style=svgCanvas.getStyle();
    ...
    '<text font-size="14" y="14" x="0">'+name+'</text>'+
   '</marker>'+
   '<marker id="StartRelation'+name+'" viewBox="0 0 80 80" refX="0" refY="0" markerUnits="strokeWidth" markerWidth="80" markerHeight="80" >'+
 //   '<path d="m0,0l20,10l-20,10l0,-20z"/>'+
   ...
   //from and to will be the same, we'll move down and right
   //circle radius is 11
...
// we want to draw an arc not a line
// a30,50 0 0,1 100,100"
// first two are x and y radius
// third is the x-axis-rotation, leave set to zero
// fourth large arc or small arc flag
// fifth clockwise or ant
// six and seven to where
...
var arc='a20,20 0 1,1 10,10" ';
    ...
    relation= '<path d="m'+mx+','+my+
            ...
            arc+        //as to have a mid-point (vertex) //marker-mid="url(#Relation'+name+')"
            'id="'+iD+'" '+
            'marker-end="url(#Triangle)" marker-start="url(#StartRelation'+name+')"  fill="none"  stroke="black"/>';
                  ...
                  //          var choice="";
                            ...
                            if (from&&to){
                                ...
                                if(reply2===reply3)  //relation to itself
                                    relation=relationsMF.makeCircularRelation(reply1,from,to);
                                else
                                    relation=relationsMF.makeRelation(reply1,from,to);
                                
                               if (!gInterpretationBoard.isRelationDuplicate(reply1,reply2,reply3)){
